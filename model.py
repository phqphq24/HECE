...
class Utt_Classify(nn.Module):
    ...
    def forward(self, hidden_state, bert_clause_idx_b, emotion_b, relative_pos_b, adj_b, conv_len_b, emotion_embedding, emotion_list, pos_embedding):
        ...
        conv_sents_h = self.gnn(conv_sents_h, conv_len_b, adj_b)  # batch, conv_len, feat_dim
        conv_emo_utt = []
        ...
        emotion_embed = emotion_embedding(emotion_id)  # batch, emo_dim
        emotion_embed_ = emotion_embed.unsqueeze(1).expand(-1, conv_len, -1) # batch, conv_len, emo_dim

        relative_pos_b = relative_pos_b.to(DEVICE)
        pos_embed = pos_embedding(relative_pos_b) # batch, conv_len, pos_dim
        kernel_rel_pos_b = self.kernel_generator(relative_pos_b)
        pos_embed = torch.matmul(kernel_rel_pos_b, pos_embed)
        conv_sents_h_ = torch.cat([emotion_embed_, conv_sents_h, pos_embed], dim=-1) # batch, conv_len, emo_dim+feat_dim+pos_dim
    ...
    def kernel_generator(self, relative_pos_b):
        ...
        return torch.stack(kernel_rel_pos_b, dim=0) # batch, seq_len, seq_len
...
class Span_Classify(nn.Module):
    ...
    def forward(self, hidden_state, bert_word_idx_b, emotion_b, conv_clause_len_b, conv_len_b, emotion_embedding, emotion_list, utt_prob, conv_emo_utt, pos_embedding, relative_pos_b):
        ...
        conv_words_h = self.batched_index_select(hidden_state, bert_word_idx_b)  # batch, conv_word_num, feat_dim
        ...
        utt_pred_label = self.build_utt_pred_mask(utt_prob, conv_clause_len_b, conv_len_b, max_conv_word_len) # batch, conv_word_num
    ...
    def build_utt_pred_mask(self, utt_prob, conv_clause_len_b, conv_len_b, max_conv_word_len):
        # utt_prob: batch, conv_len
        # conv_clause_len_b: batch个list, 每个list中是对应的对话中每条句子的长度
        ...
        for i in range(batch):
            ...
            for j in range(temp_conv_len):
                if logistic(utt_prob[i][j]) > 0.4:  # 0.5 0.3 0.33 0.4
        ...
        return torch.LongTensor(utt_pred_mask)  # batch, max_conv_word_len
